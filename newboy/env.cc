...
static newboy::Logger::ptr g_logger = NEWBOY_LOG_NAME("system");
    ...
    for(int i = 1; i < argc; ++i) {
        if(argv[i][0] == '-') {
            if(strlen(argv[i]) > 1) {
                ...
                NEWBOY_LOG_ERROR(g_logger) << "invalid arg idx=" << i
                    << " val=" << argv[i];
                return false;
            }
        } else {
...
std::string Env::getAbsoluteWorkPath(const std::string& path) const {
    ...
    static newboy::ConfigVar<std::string>::ptr g_server_work_path =
        newboy::Config::Lookup<std::string>("server.work_path");
