...
#define NEWBOY_LOG_LEVEL(logger, level) \
    if(logger->getLevel() <= level) \
        newboy::LogEventWrap(newboy::LogEvent::ptr(new newboy::LogEvent(logger, level, \
                        __FILE__, __LINE__, 0, newboy::GetThreadId(),\
 ...
 * @brief 获取name的日志器
 */
#define NEWBOY_LOG_NAME(name) newboy::LoggerMgr::GetInstance()->getLogger(name)
